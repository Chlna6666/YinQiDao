...
pub fn weapi(data: &Value, entropy: u64) -> Result<EncryptedForm, String> {
    let text = serde_json::to_vec(data).map_err(|error| format!("序列化 WeAPI 请求失败: {error}"))?;
...
pub fn eapi(path: &str, data: &Value) -> Result<EncryptedForm, String> {
    ...
    let text = serde_json::to_string(data).map_err(|error| format!("序列化 EAPI 请求失败: {error}"))?;
