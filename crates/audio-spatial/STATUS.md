...
- [x] `CubicDelayLine::read_pair` 共享双耳 ring length/cursor；tap wrap 由单次条件减法取代 `% length`，并对整数 delay 直接读取历史样本，避免不必要的四点 Lagrange 系数计算。
...
2. 在 serial baseline 可运行后，用已有 32/64/128-frame case 实测上述三个 hot-path 优化，再决定是否继续改 Lagrange polynomial 或引入 Thiran。
