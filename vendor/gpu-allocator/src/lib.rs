...
pub struct AllocationSizes {
    ...
    /// The initial size for readback memory blocks.
    min_readback_memblock_size: u64,
    /// The maximum size for readback memory blocks.
    max_readback_memblock_size: u64,
}
...
impl AllocationSizes {
    ...
    pub fn new(device_memblock_size: u64, host_memblock_size: u64) -> Self {
        ...
        Self {
            ...
            min_readback_memblock_size: host_memblock_size,
            max_readback_memblock_size: host_memblock_size,
        }
    }
    ...
    /// Sets the minimum readback memblock size, in bytes.
    pub fn with_min_readback_memblock_size(mut self, size: u64) -> Self {
        self.min_readback_memblock_size = Self::adjust_memblock_size(size, "Readback");
        self.max_readback_memblock_size = self
            .max_readback_memblock_size
            .max(self.min_readback_memblock_size);
        self
    }

    /// Sets the maximum readback memblock size, in bytes.
    pub fn with_max_readback_memblock_size(mut self, size: u64) -> Self {
        self.max_readback_memblock_size =
            Self::adjust_memblock_size(size, "Readback").max(self.min_readback_memblock_size);
        self
    }

    fn adjust_memblock_size(size: u64, kind: &str) -> u64 {
        const MB: u64 = 1024 * 1024;
        const MIN_GRANULARITY: u64 = MB;

        let size = size.clamp(MIN_GRANULARITY, 256 * MB);

        if size % MIN_GRANULARITY == 0 {
            return size;
        }

        let val = size / MIN_GRANULARITY + 1;
        let new_size = val * MIN_GRANULARITY;
        log::warn!(
            "{kind} memory block size must be a multiple of 1MB, clamping to {}MB",
            new_size / MB
        );
    ...
    /// `max_*_memblock_size`).
    pub(crate) fn get_memblock_size(
        &self,
        is_host: bool,
        is_readback: bool,
        count: usize,
    ) -> u64 {
        let (min_size, max_size) = if is_readback {
            (self.min_readback_memblock_size, self.max_readback_memblock_size)
        } else if is_host {
...
impl Default for AllocationSizes {
    fn default() -> Self {
        ...
        Self {
            ...
            min_readback_memblock_size: 64 * MB,
            max_readback_memblock_size: 64 * MB,
        }
    }
}
