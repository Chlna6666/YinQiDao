...
pub struct MemoryLocationReport {
    ...
    pub committed_allocated_bytes: u64,
    pub committed_allocation_count: usize,
}

#[derive(Clone, Debug, Default)]
pub struct D3d12AllocatorReport {
    pub gpu_only: MemoryLocationReport,
    pub cpu_to_gpu: MemoryLocationReport,
    pub gpu_to_cpu: MemoryLocationReport,
}
...
impl MemoryType {
    fn allocate(
        ...
        let is_readback = self.memory_location == MemoryLocation::GpuToCpu;
...
impl Allocator {
    pub fn generate_detailed_report(&self) -> D3d12AllocatorReport {
        let mut report = D3d12AllocatorReport::default();

        for memory_type in &self.memory_types {
            let bucket = match memory_type.memory_location {
                MemoryLocation::GpuOnly => &mut report.gpu_only,
                MemoryLocation::CpuToGpu => &mut report.cpu_to_gpu,
                MemoryLocation::GpuToCpu => &mut report.gpu_to_cpu,
                MemoryLocation::Unknown => continue,
            };

            bucket.committed_allocated_bytes = bucket
                .committed_allocated_bytes
                .saturating_add(memory_type.committed_allocations.total_size);
            bucket.committed_allocation_count = bucket
                .committed_allocation_count
                .saturating_add(memory_type.committed_allocations.num_allocations);

            for block in memory_type.memory_blocks.iter().flatten() {
                bucket.reserved_bytes = bucket.reserved_bytes.saturating_add(block.size);
                bucket.block_count = bucket.block_count.saturating_add(1);
                bucket.allocated_bytes = bucket.allocated_bytes.saturating_add(
                    block.sub_allocator
                        .report_allocations()
                        .iter()
